...
// PM Modif: bing map ID
        bingMapId += "_GPS";
        if(this.selfManagedInstrument) {
            // We directly add the instrument identifier 
            if (typeof (arg) === "string") {
                bingMapId += "_" + arg;
            }
        }
        else {
            if(this.instrument.gpsType)
                bingMapId += "_" + this.instrument.gpsType;
            if(this.instrument.isPanel)
                bingMapId += "_Panel";
            if(this.instrument.urlConfig.index)
                bingMapId += "_" + this.instrument.urlConfig.index;
        }
// PM Modif: End bing map ID
...
//PM Modif: special instrument                
        this.updateSize(this.selfManagedInstrument);
//PM Modif: End special instrument                
        if (this.selfManagedInstrument) {
...
// PM Modif: Trackup
                let bingRadius = this.navMap.NMWidth * 0.5 * this.rangeFactor * this.mapScaleFactor;
// PM Modif: End Trackup
